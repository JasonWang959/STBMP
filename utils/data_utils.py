...
def readCSVasFloat(filename):#读取CSV文件转成矩阵
    """
    ...
    """
    returnArray = []
    lines = open(filename).readlines()#读取文件数据
    for line in lines:
...
def define_actions(action):#动作标签定义
    """
...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    #print(sampled_seq.shape)#b,20,114
...
def expmap2xyz_torch(expmap):#扩展映射准换成XYZ坐标
    """
